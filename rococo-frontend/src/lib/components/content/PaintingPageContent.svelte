...
<article class="card m-8 p-4 min-h-[90%]">
    <div class="grid grid-cols-1 lg:grid-cols-2 m-4 min-h-full">
        <img data-testid="painting-photo" src={$singlePaintingStore?.data?.content} alt={$singlePaintingStore?.data?.title}
             class="my-4 mx-auto w-full"/>
        <div>
            <div>
                <header class="card-header text-center font-bold text-3xl">{$singlePaintingStore?.data?.title}</header>
                <div data-testid="artist-name" class="text-center">{$singlePaintingStore?.data?.artist?.name}</div>
                <div data-testid="museum-title" class="text-center italic">{$singlePaintingStore?.data?.museum?.title}</div>
            ...
            <p data-testid="painting-description" style="white-space: pre-wrap;" class="m-4">{$singlePaintingStore?.data?.description}</p>
        </div>
    </div>
</article>