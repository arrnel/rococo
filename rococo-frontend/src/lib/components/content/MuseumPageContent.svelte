...
<article class="card m-8 p-2 min-h-[90%]">
    <div class="grid grid-cols-1 lg:grid-cols-2 m-4 min-h-full">
        <div class="mr-4">
            <header class="card-header text-center font-bold text-3xl">{$singleMuseumStore.data?.title}</header>
            ...
            <div style="white-space: pre-wrap;" data-testid="museum-description">{$singleMuseumStore.data?.description}</div>
        </div>
