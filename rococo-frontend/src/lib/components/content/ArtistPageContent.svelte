        ...
        if (resData) {
            ...
            singleArtistStore.update((prevState) => {
                return {
                    ...
                    noMoreData: resData.data ? page === resData.data?.totalPages - 1 : true,
                    isLoading: false,
                }
            });
        }
...
<article class="card m-8">
    <header data-testid="artist-name" class="card-header text-center font-bold text-3xl pt-10">{$singleArtistStore?.data?.name}</header>
    <section class="p-4 grid grid-cols-1 lg:grid-cols-3 justify-items-center">
        <div class="flex flex-col">
            ...
            {#if $sessionStore.user}
                ...
                />
                <button class="btn variant-filled-primary m-3 mx-auto block w-full" type="button"
                        on:click={clickAddButton}>Добавить картину
                </button>
            {/if}
        </div>
        <p style="white-space: pre-wrap;" data-testid="artist-biography" class="col-span-2 w-4/5 m-2">{$singleArtistStore?.data?.biography}</p>
    </section>
    <section class="p-4">
        {#if $singleArtistStore.paintings && $singleArtistStore.paintings.length > 0}
            <p class = "ml-4 font-bold text-xl" data-testid="artist-paintings-title">Картины художника</p>
        {/if}
        <ListWrapper data={$singleArtistStore.paintings}
                     isSearchNotEmpty={false}
                     dataTestId="artist-paintings"
                     emptyDataTestId="empty-artist-paintings"
