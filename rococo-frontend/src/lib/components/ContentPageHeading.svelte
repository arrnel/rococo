...
<div class="flex items-center justify-between m-4">
    ...
    {#if $sessionStore.user}
        <button type="button" class="btn variant-filled-primary ml-4"
                on:click={onAddButtonClick}>{addButtonName}</button>
    {/if}
</div>

<div class="flex justify-center mb-4 mx-8" data-testId="search">
    <input class="input" bind:value={search} title={searchPlaceholder} type="search" placeholder={searchPlaceholder}
           on:keypress={(evt) => {
        if(evt.key === "Enter") {
            loadFunction(search);
        }
